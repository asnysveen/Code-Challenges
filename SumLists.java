...
public class SumLists {
	...
	public static void main(String[] args) {
		...
		List<Integer> A = new LinkedList<Integer>(Arrays.asList(2, 1, 3));
		List<Integer> B = new LinkedList<Integer>(Arrays.asList(4, 8, 5));

		int lenA = A.size();
		int lenB = B.size();

		// Prints addition of forward ordered lists
		System.out.println(intToList(add(listToInt(A, lenA), listToInt(B, lenB))));
		
		// Prints addition of reverse ordered lists
		System.out.println(intToList(add(revListToInt(A, lenA), revListToInt(B, lenB))));
	}
